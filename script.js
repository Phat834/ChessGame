...
capture: function (target) {
      ...
      // Check for win after capture
      return main.methods.checkWin();
    },
...
togglehighlight: function(options) {
    ...
    checkWin: function() {
      if (main.variables.pieces.w_king.captured) {
        main.methods.showWinner('Black');
        return true;
      } else if (main.variables.pieces.b_king.captured) {
        main.methods.showWinner('White');
        return true;
      }
      return false;
    },
    ...
    showWinner: function(winner) {
      ...
      $('#turn').addClass('winner-highlight');
    },

    resetGame: function() {
      main.variables.pieces = JSON.parse(JSON.stringify(main.variables.init));
      main.variables.turn = 'w';
      main.variables.selectedpiece = '';
...
let t5 = (target.id == '7_8');
       ...
       if (selectedpiece.id != target.id && main.variables.highlighted.indexOf(target.id) != (-1)) { // if it's not trying to capture pieces not in its movement range
         ...
         // capture
         const gameEnded = main.methods.capture(target)
         if (!gameEnded) {
           main.methods.endturn();
         }
